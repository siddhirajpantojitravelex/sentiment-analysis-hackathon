const QUERIES = {
    ...
    SENTIMENT: {
        ...
        UPDATE: 'UPDATE SENTIMENT SET prod_value = $2, pros_magnitude = $3, cons_value = $4, cons_magnitude = $5 ,result = $6 WHERE review_id = $1 RETURNING * ',
    ...
    SENTIMENT_CLASSIFICATION: {
        ...
        INNER_GRAPH: "SELECT COUNT(*) as cnt, (case when result > 1 then 'Positive' when result <0 then 'Negative' else 'Neutral'  end ) as category FROM sentiment where review_id in ( SELECT review_classification_id from sentiment_classification where classification_description = $1 ) group by category "
    }
};
